...
export const CustomerManager = () => {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>, saleId: string) => {
    ...
    try {
      ...
      const fileName = `${shopId}/${selectedCustomer.id}/${saleId}_${Math.random().toString(36).substring(2)}.${fileExt}`;
        ...
        <div className="w-full md:w-96 flex flex-col gap-4">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-4 bg-white rounded-2xl shadow-sm font-bold text-sm focus:ring-2 focus:ring-indigo-500 outline-none border-none transition-all" 
              value={searchQuery} 
          ...
          <div className="flex-1 overflow-y-auto bg-white rounded-[2.5rem] shadow-sm p-3 space-y-1 border border-slate-50 custom-scrollbar">
            {filteredCustomers.length > 0 ? filteredCustomers.map(c => (
              <button key={c.id} onClick={() => handleSelectCustomer(c)} className={`w-full p-4 rounded-2xl text-left transition-all flex items-center justify-between group ${selectedCustomer?.id === c.id ? 'bg-slate-900 text-white shadow-xl' : 'hover:bg-slate-50 text-slate-600'}`}>
              ...
              <div className="py-20 text-center opacity-20">
                ...
                <p className="text-[10px] font-black uppercase">No Clients Found</p>
              </div>
            )}
          </div>
        </div>

        {/* 右側：詳細・ビジュアルカルテ（変更なしのため一部省略可ですが全文維持） */}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                          <div className="relative w-full md:w-56 h-56 rounded-[2.5rem] overflow-hidden bg-slate-50 border border-slate-100 flex-shrink-0 group/photo shadow-inner">
                            {sale.image_url ? (
                              ...
                              <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 transition-colors">
                                {isUploading === sale.id ? <Loader2 size={32} className="text-indigo-500 animate-spin" /> : <><ImageIcon size={32} className="text-slate-300 mb-2" /><span className="text-[9px] font-black text-slate-400 tracking-widest uppercase">写真を追加</span></>}
                                <input type="file" className="hidden" accept="image/*" onChange={(e) => handlePhotoUpload(e, sale.id)} />
                              </label>
                            )}
                          </div>
                          <div className="flex-1 bg-slate-50/50 p-8 rounded-[2.5rem] border border-slate-100 hover:bg-white hover:shadow-xl transition-all duration-300">
                            <div className="flex items-center gap-2 mb-4 text-slate-400"><History size={14} /><span className="text-[10px] font-black uppercase tracking-widest">Service Note</span></div>
                            <p className="text-xs font-bold text-slate-600 leading-relaxed italic">{sale.memo || "施術メモは登録されていません。"}</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/40 backdrop-blur-md p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-[3.5rem] p-12 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <InputGroup label="住所" value={newCustomer.address} onChange={(v: string) => setNewCustomer({...newCustomer, address: v})} placeholder="東京都..." />
            </div>
            <button onClick={handleAddCustomer} className="w-full mt-10 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-xs tracking-[0.3em] hover:bg-indigo-600 transition-all shadow-2xl flex items-center justify-center gap-2 group">プロフィール作成 <Plus size={16} className="group-hover:rotate-90 transition-transform"/></button>
          </div>
        </div>
      )}
    </div>
